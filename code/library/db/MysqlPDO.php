...
public function __construct(array $config)
{
	...
	if(!class_exists("PDO")) throw new Exception('PHP环境未安装PDO函数库！');
...
private function conn(){
	...
	try {
		$this->conn =  new PDO(
				...
				array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES {$this->_config['charset']}")
		);
...
{
    return $this->update($table, $conditions, array($field=>$value));
}
...
{
	return $this->delete($table, array('id'=>$pk));
}
...
private function __prepera_format($table, $rows)
{
	...
	public function getTable($tbl_name){
        ...
        if (empty($tableInfo)) throw new Exception($tbl_name.'不存在');
        return $tableInfo;
	}
